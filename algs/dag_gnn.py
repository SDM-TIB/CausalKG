...
## modified from https://github.com/huawei-noah/trustworthyAI/blob/master/gcastle/castle/algorithms/gradient/dag_gnn/torch/dag_gnn.py
...
class DAG_GNN(BaseLearner):
    ...
    def __init__(self, encoder_type='mlp', decoder_type='mlp',
                 ...
                 optimizer='adam', seed=42, device_type='cpu', device_ids='0', edge_mask=None):
    ...
    def learn(self, data, columns=None, **kwargs):
        ...
        if self.edge_mask is not None:
            self._edge_mask = torch.tensor(self.edge_mask, dtype=adj_A.dtype, device=self.device)
        else:
    ...
    def _train(self, train_loader, optimizer, lambda_a, c_a):
        ...
        for batch_idx, (data, relations) in enumerate(train_loader):
            ...
            if self._edge_mask is not None:
                self.encoder.adj_A.data.mul_(self._edge_mask)
            self.scheduler.step()
