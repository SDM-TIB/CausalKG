"""The ``ckg:`` vocabulary (``http://sdm-causalkg.org/``) shared by Plan 1 and Plan 2.

Plan 1 minted a small vocabulary for the discovered ontological causal graph
(``ckg:OntologicalCausalGraph``, ``ckg:PropertyNode``, ``ckg:CausalEdge``, ...)
inline in :mod:`causalkg.result`.  Plan 2 roughly triples it with the terms
...
module is the single source of truth for all of it; :mod:`causalkg.result`
re-exports the Plan 1 subset so existing imports keep working.

**The namespace does not dereference.**  ``http://sdm-causalkg.org/`` is not a
...
terms it describes: ``causalkg/ontology.ttl`` (a committed snapshot, written by
``python -m causalkg.vocab``) and the service's ``GET /vocab``.  Point the
...
CKG = Namespace("http://sdm-causalkg.org/")
...
#:
#: * ``ckg:performance`` -> ``mls:hasQuality`` / ``mls:EvaluationMeasure``: mls
#:   reifies a measure as a resource, ckg carries a bare value plus
#:   ``ckg:performanceMetric``.  Not the same shape; an alignment would be a lie.
#: * ``ckg:usedModel`` -> ``prov:wasInfluencedBy``: prov's term is so broad it
...
#:   ``ckg:forQuery``/``ckg:usedModel``; that is enough.
#: * ``ckg:library`` / ``ckg:libraryVersion`` / ``ckg:trainingRows``: no exact
...
# ---------------------------------------------------------------------- #
NODE_STEM = "http://sdm-causalkg.org/node/"
GRAPH_STEM = "http://sdm-causalkg.org/graph/"
RUN_STEM = "http://sdm-causalkg.org/run/"                  # ckg:DiscoveryRun (Plan 1)
MODEL_STEM = "http://sdm-causalkg.org/model/"
MODEL_RUN_STEM = "http://sdm-causalkg.org/modelrun/"       # ckg:ModelFitRun (Plan 2)
QUERY_STEM = "http://sdm-causalkg.org/query/"
ANSWER_STEM = "http://sdm-causalkg.org/answer/"
INTERVENTION_STEM = "http://sdm-causalkg.org/intervention/"
CONDITION_STEM = "http://sdm-causalkg.org/condition/"
...
_VOCAB_CLASSES = {
    ...
    "Parameter": "One named hyper-parameter of a ckg:DiscoveryRun.",
}

_VOCAB_PROPERTIES = {
    "hasNode": "Property node belonging to this causal graph "
               "(the ckg:hasNodeSlot/ckg:slotNode shortcut).",
    "hasNodeSlot": "A ckg:NodeSlot of this graph: one column of adj.",
    "slotIndex": "0-based column position of a ckg:NodeSlot in adj.",
    "slotNode": "The ckg:PropertyNode occupying a ckg:NodeSlot.",
    "hasParameter": "A ckg:Parameter of this discovery run.",
    "parameterName": "Name of a hyper-parameter, e.g. 'alpha' or 'seed'.",
    "parameterValue": "Value of a hyper-parameter, as text; read with ckg:parameterKind.",
    "parameterKind": "'str' | 'int' | 'float' | 'bool' | 'null' | 'json' — how to read ckg:parameterValue.",
    ...
    "method": "Name of the causal-discovery method (GES, NOTEARS, ...). On the "
              "ckg:DiscoveryRun; reach it from a graph via prov:wasGeneratedBy.",
    ...
    "range": "Range R_p of a property node: a class when ckg:nodeKind is 'object', "
    ...
    "cause": "The ckg:PropertyNode acting as cause.",
    "effect": "The ckg:PropertyNode acting as effect.",
    ...
    "viaRelation": "Object property traversed by the edge; ckg:epsilon intra-class. "
    ...
    "viaPath": "The edge's relation label as one SPARQL 1.1 property path, e.g. "
               ...
               "information ckg:viaRelation does not: a multi-hop join, or a single "
               "inverse hop. Absent for a plain forward hop and for ckg:epsilon.",
}
...
# ---------------------------------------------------------------------- #
_LEGACY_PROPERTIES = {
    "nodeList": "Ordered rdf:List of the property nodes. Superseded by ckg:hasNodeSlot: "
    ...
    "parameters": "Hyper-parameters of the run as one JSON literal. Superseded by "
                  "ckg:hasParameter — SDM-RDFizer cannot carry a quote through a "
                  "literal, and a JSON object is all quotes.",
    "domainClass": "Superseded by ckg:domain.",
    "rangeClass": "Superseded by ckg:range.",
    "rangeDatatype": "Superseded by ckg:range (ckg:nodeKind already carries the distinction).",
    "constrained": "Duplicated ckg:usedTopologicalConstraint onto the graph. "
                   "Superseded by that term on the ckg:DiscoveryRun alone.",
    "relationLabel": "Human-readable rendering of the edge's relation. Superseded by "
                     "rdfs:label (for humans) and ckg:viaPath (for machines).",
    "relationDirection": "'forward' | 'inverse'. Superseded by the '^' operator inside ckg:viaPath.",
    "pathLength": "Number of hops. Derivable from ckg:viaPath.",
    "hop": "A step of a path. Superseded by ckg:viaPath.",
    "hopIndex": "1-based position of a hop. Superseded by the order inside ckg:viaPath.",
    "hopRelation": "Object property traversed by a hop. Superseded by ckg:viaPath.",
    "hopDirection": "'forward' | 'inverse' traversal of a hop. Superseded by ckg:viaPath.",
}
_LEGACY_CLASSES = {
    "RelationPath": "An ordered multi-hop relation label, reified. Superseded by the "
                    "ckg:viaPath property-path literal.",
    "RelationHop": "One step inside a ckg:RelationPath. Superseded by ckg:viaPath.",
}
...
_VOCAB_CLASSES.update({
    ...
    "ModelFitRun": "One execution of CausalModel.fit that produced a ckg:CausalModel.",
...
_VOCAB_PROPERTIES.update({
    "overGraph": "The ckg:OntologicalCausalGraph this model was fitted on.",
    ...
    "hasMechanism": "A ckg:MechanismAssignment belonging to this model, one per node.",
    "forNode": "The ckg:PropertyNode a mechanism assignment describes.",
    ...
    "performanceMetric": "Name of the metric ckg:performance is measured in (F1, R2, ...).",
})
...
# ---------------------------------------------------------------------- #
_VOCAB_CLASSES.update({
    "CausalQuery": "Abstract: a query against a ckg:CausalModel. The query IRI is also the world IRI.",
...
_VOCAB_PROPERTIES.update({
    "usedModel": "The ckg:CausalModel that answered this query.",
    "aboutEntity": "Counterfactual only: the single entity the hypothetical world is about.",
    "targetNode": "The ckg:PropertyNode targeted as the outcome (1..n).",
    "targetProperty": "Derived shortcut: the property IRI of ckg:targetNode.",
    "hasIntervention": "A ckg:Intervention applied in this query (1..n; several = joint intervention).",
    "referenceIntervention": "Interventional only: the baseline arm of a contrast.",
    "hasEvidence": "Conditional only: a ckg:Condition read as observed evidence.",
    ...
    "onNode": "The ckg:PropertyNode an Intervention/Estimate targets.",
    "onProperty": "Derived shortcut: the property IRI of ckg:onNode.",
    ...
    "hasQuery": "Entity-first navigation: a ckg:CausalQuery this entity is the subject of.",
})
...
# ---------------------------------------------------------------------- #
_VOCAB_CLASSES.update({
    "CausalEstimate": "Abstract: the answer to a ckg:CausalQuery.",
    "ConditionalEstimate": "Answer to a ckg:ConditionalQuery.",
    "InterventionalEstimate": "Answer to a ckg:InterventionalQuery.",
    "CounterfactualEstimate": "Answer to a ckg:CounterfactualQuery.",
    "IntervalEstimate": "Uncertainty hook for a continuous predictedValue: a credible interval.",
})
_VOCAB_PROPERTIES.update({
    "forQuery": "The ckg:CausalQuery (== hypothetical world) this estimate answers.",
    "predictedValue": "The answer: one modality-polymorphic value term for both categorical and continuous targets.",
    "factualValue": "Counterfactual only: what ckg:onNode actually *was* for ckg:onEntity in "
                    "the source KG — the baseline ckg:predictedValue is a counterfactual to. "
    ...
    "causalEffect": "Interventional only: contrast against ckg:referenceIntervention.",
    "diagnostics": "Optional hook (0..1): a JSON literal of backend/samples/ESS/coupling — run diagnostics, not world facts.",
    "lowerBound": "Lower bound of a ckg:IntervalEstimate.",
    "upperBound": "Upper bound of a ckg:IntervalEstimate.",
    "credibleLevel": "Credible level (e.g. 0.95) of a ckg:IntervalEstimate.",
    "probability": "Probability of the ckg:predictedValue actually reported — i.e. of the "
    ...
    "standardDeviation": "Spread of a continuous ckg:predictedValue across the draws it was "
                         "summarised from. Read it together with ckg:rowCount, which says "
    ...
    "rowCount": "How many flat-join rows the unit of this world occupied. 1 means any "
                "ckg:standardDeviation is purely counterfactual uncertainty (propagated from "
...
# estimate: a ckg:CategoricalDistribution resource with one ckg:Outcome child
...
# Those now sit directly on the ckg:CounterfactualEstimate as ckg:predictedValue
# and ckg:probability, so the whole distribution sub-tree is gone.
#
...
_RETIRED_CLASSES = {
    "CategoricalDistribution": "Full outcome distribution of a categorical predictedValue. "
                               "Superseded by ckg:probability on the estimate itself, which "
                               "carries the arg-max level's mass and nothing else.",
    "Outcome": "One (value, probability) pair inside a ckg:CategoricalDistribution. "
               "Superseded along with it.",
}
_RETIRED_PROPERTIES = {
    "uncertainty": "Hook from an estimate to a ckg:IntervalEstimate or "
                   "ckg:CategoricalDistribution. The categorical half is superseded by "
                   "ckg:probability; no estimate ever carried the interval half.",
    "outcome": "A ckg:Outcome belonging to a ckg:CategoricalDistribution. Superseded.",
    "outcomeValue": "The class label of a ckg:Outcome. Superseded by ckg:predictedValue, "
                    "which now names the arg-max level directly.",
}

CKG.epsilon  # noqa: B018  (touch the term so it's bound below)
...
#: Alignments to foreign vocabularies, as ``(ckg term, predicate, foreign term)``.
...
_ALIGNMENTS = [
    (CKG.CausalModel, RDFS.subClassOf, MLS.Model),
    (CKG.ModelFitRun, RDFS.subClassOf, MLS.Run),
]


def vocabulary(graph: Optional[Graph] = None) -> Graph:
    """The full ``ckg:`` schema (Plan 1 + Plan 2) describing every term used above.

    Includes the foreign alignments in :data:`_ALIGNMENTS`, so a consumer that
    already speaks ML Schema can classify a ``ckg:CausalModel`` without reading
    ...
    g.add((CKG.epsilon, RDFS.label, Literal("ε")))
    g.add((CKG.epsilon, RDFS.comment, Literal(
